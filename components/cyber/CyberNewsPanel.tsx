...
function NewsCard({ item, isSelected, onClick }: { item: CyberNewsItem; isSelected: boolean; onClick: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        background: isSelected ? "var(--bg-surface-active)" : hovered ? "var(--bg-surface-hover)" : "var(--bg-surface)",
        border: isSelected ? "1px solid var(--accent-green-border)" : hovered ? "1px solid var(--border-hover)" : "1px solid var(--border-subtle)",
    ...
    >
      ...
      {isSelected && (
        <div className="absolute left-0 top-0 bottom-0 w-[2px]" style={{ background: "var(--accent-green)", borderRadius: "var(--radius-md) 0 0 var(--radius-md)" }} />
      )}
      {/* Headline */}
      ...
      <div className="flex items-center gap-1.5">
        <span className="px-1.5 py-0.5 rounded" style={{ fontSize: "var(--fs-2xs)", fontWeight: 600, color: "var(--accent-green)", background: "var(--accent-green-bg)", border: "1px solid var(--accent-green-border)" }}>
          {item.categoryTag}
        </span>
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--bg-panel)", border: "1px solid var(--border-primary)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
      ...
      <div className="flex items-center justify-between flex-shrink-0 px-3.5 py-2.5" style={{ borderBottom: "1px solid var(--border-dim)" }}>
        ...
        <div className="flex items-center gap-1.5">
          <span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--accent-green-dot)", animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: "var(--fs-xs)", color: "var(--accent-green-dim)", fontWeight: 600 }}>LIVE</span>
        </div>
      </div>
      {/* Feed */}
